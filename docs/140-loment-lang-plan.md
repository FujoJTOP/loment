...
### 11.5 进度（2026-09-09，P9/P10 主体）

| 里程碑 | 交付 | 证据 |
|---|---|---|
| M89/M90 | 25 个示例 + L0 单源库复用 | `loment_p9_test` |
| M91/M94/M95 | 版本策略 / Rust→Loment 迁移指南 / 发布检查单 | docs/151 |
| M92 | aarch64 交叉编译（`elf64-littleaarch64`） | 未执行（无模拟器） |
...
| M96/M100 | 1.0 冻结与发布审计 | ⚠️ 未达（1.0-pre） |
| **门禁** | `ci.py --static-only` 10 项（+`loment_p9_test`） | **10/10** |

里程碑进度 **80/100**（9 项部分：M48/M50/M56/M59/M68/M70/M71/M74/M92；
未做：M80–M88 自举主体、M96 冻结、M100 发布审计）。

### 11.6 进度（2026-09-09，M80 部分）

| 里程碑 | 交付 | 证据 |
|---|---|---|
| M80 | Loment 版 parser（语句/表达式子集） | 5 个真实文件的 AST dump 与 Python 版逐字符一致 |
| **门禁** | `ci.py --static-only` 10 项 | **10/10** |

**里程碑矩阵（由 `tools/loment_status.py` 从 docs/145 生成，见 docs/154）**：
完成 **70** / 部分 **13** / 未达 **2**（M96 冻结、M100 发布审计）/ 未开始 **15**（M81–M88 自举主体等）。
