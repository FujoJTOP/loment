...
| 规范 / 实现分离 | ~~规范公开 + 实现闭源~~ → **规范与实现一并开源**（§10-2） | 不变（Loment 不碰这层） |
...
| 开源策略 | 全开源（与项目一致） | **规范与实现一并开源**（§10-2，2026-09-08 用户指示，覆盖 docs/110 §4 原文；`docs/142` §1 同记） |
| 主要风险 | 零语料（→ Rust 子集语法对冲） | 内核侵蚀（**原对冲"规范/实现分离"已随实现开源放弃** —— §10-2 明写这是代价；剩下的只是先发与规范定义权，**不是技术闸门**） |

**一句话**：Loment 管"怎么写"，Potato 管"怎么看"。两者相遇于**语义模型**——Loment 编译器
必须为每个编译单元产出 Potato 形式对象。

### 9.3 为什么不能合并（原两条，现**一条**）

> **第 1 条已失效**（2026-09-17 记账）。它替一个死掉的反对意见说话了：那理由是"Potato 是
> 唯一闭源层，合并会造出闭源语言实现"，而**闭源这个前提 2026-09-08 就被 §10-2 覆盖了**
> （规范与实现一并开源，`docs/142` §1 同记）。保留原文是因为它记着当时的推理，但读者
> 不要拿它当理由用 —— **"不合并"这个结论现在只由第 2 条撑着**，下面也照这个口径写。

1. ~~**闭源决策污染**：Potato 是项目唯一刻意闭源层。若 Potato 变成语言，闭源的就成了语言
   实现——没有内核敢构建在闭源编译器上（包括本项目自己）。作为"表示层 + 校验器"，闭源
   实现可接受（公开规范的私有校验器，有先例）。~~
   **失效**：见上。这条的两个前提（"唯一闭源层"、"实现闭源"）都不再成立。
2. **零语料决策污染**：语言必须有语料才能被 Agent 书写；表示层不需要语料，它消费结构。
   合并等于把 Loment 拖回 docs/110 §1 已否决的"世俗新语言"陷阱。

**这条剩下的理由够不够，要说实话**：够，但**它是"实践理由"不是"逻辑禁令"** —— 它说的是
"合了就要给 Potato 补一套语料，而那正是 docs/110 §1 否决过的路线"。所以将来若有人真想
合并，**要驳的不是这条理由，是 docs/110 §1 那个否决**。§9.4/§9.5 的接缝规范仍然成立
（它与是否合并无关：两件事之间的映射规范总要写）。
