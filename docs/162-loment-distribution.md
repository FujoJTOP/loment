...
> 版本 `0.1.4-alpha2`（显示名 **Loment 0.1.4 Alpha2**）· 构建器 `tools/loment_dist.py`
...
| 文件 | 装法 | 说明 |
|---|---|---|
| `loment-0.1.4-alpha2-linux-x64.tar.gz` | 解包 → `sh install.sh` | Linux / WSL；含 `install.sh` |
| `loment-0.1.4-alpha2-windows-x64.zip` | 解包 → `powershell -File install.ps1` | Windows；含 `install.ps1` / `install.cmd` |
| `loment-0.1.4-alpha2-windows-x64-setup.exe` | **双击** | 自解压安装包（Windows 自带 `iexpress` 做的） |
...
| `share/loment/version` | `Loment 0.1.4 Alpha2 (0.1.4-alpha2)` + 提交号与提交日期 |
