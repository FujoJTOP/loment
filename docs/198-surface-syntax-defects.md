...
> 是边界说明 —— 四条缺陷与它是两件事）。
>
> **本文件只上报，没有动任何一份语言线的实现** —— 四条里有三条的修法都会碰到翻译器与
> 检查器（`tools/pytrans.py` / `tools/potato_from.py` / `tools/lomentc.py`），而那几份
> 正被并行改动，从外面动它们只会撞车。**归属与优先级由语言线定。**
>
> **状态（2026-09-20 当晚）**：**§1 已修**（语言线 `2b523f2` + `9d677b8`，四处一起改，
> 判据 `loment_pytrans_test` 4/4、`loment_trans_test` 8/8）；**§2 / §3 / §4 仍未修**。
> §1 保留原样并加了"已修"的记录 —— 它记的是**一个缺陷从发现到关上**的完整样子。

---

## 1. `~` 能过翻译器、过不了词法器（**已修**，2026-09-20，语言线 `2b523f2`）

> **状态：修完了。** 语言线按"一次改四处"落的（`2b523f2` 手写 + `9d677b8` 生成物）：
>
> | 落点 | 改了什么 |
> |---|---|
> | `tools/pytrans.py` 的 `raw()` | `ast.Invert` 从"映成 `~`"改成**点名拒**，文案给全（点出按宽度、给出两条出路） |
> | 同上的文件头两处 | 「子集」那节把 `~` 从表达式挪进**拒收**那一列；"已上报未修"由两条改成一条 |
> | `loment/pytrans/unary_invert.py` | **新增的拒收语料**，照 `bool_as_int.py` 的先例 |
> | 孪生 `lomtrans --lang python` + `loment_trans_test.py` | 跟着拒；语料里 `py_unary` 去掉 `~`、另加 `py_invert` |
>
> **判据**：`loment_pytrans_test` **4/4** —— 新那条钉两半：报得出理由（要提到"宽度"）
> **并且** `-a`/`+a` 照旧收（这条最容易过头的方向是顺手把一元那一族一起拒掉）；
> `loment_trans_test` **8/8** —— Python 66 份逐字节相同、自举链 IR 也逐字节相同。
>
> **为什么这份报告里留着它**：它记的是"**一个缺陷从发现到关上**"的完整样子 ——
> 尤其是**为什么现有的判据照不到它**（见下），那是这一条最值钱的部分。
...
| | 缺陷 | 命中面 | 该动谁 | 状态 |
|---|---|---|---|---|
| 1 | `~` 发出来编不过 | 每一份用到 `~` 的 Python 写法 | `tools/pytrans.py` + 孪生 | **已修**（语言线 `2b523f2`） |
| 2 | 声明关键词写错静默退回 | 每一份想用别的拼法、但第一行写歪的源 | `tools/potato_from.py` | **未修**（低成本，建议先做） |
| 3 | 内建两个答案 | 语句位置的内建调用 | `tools/pytrans.py` | **未修**（一处判决） |
| 4 | 依赖正文不查 | **所有库** | `tools/lomentc.py` | **未修**（最重要，但动检查器核心，另开） |

3 是把一条判决收拢到一处；4 值得单独立一项
（它同时是"库系统靠什么保证"的问题，`docs/168` / `docs/183` 那条线会关心）。

**1 的"为什么一直没被逼出来"值得单记一行**：`loment/pytrans/*.py` 那批语料里
**一个 `~` 都没有**，所以运行时判据照不到这一格 —— 是 LumtUI 那条线（不走 Python
写法就撞不到）把它拱出来的。修的时候同时补了**拒收语料**，那条缺点才算堵上：
**"上游拒了"这件事本身要有判据钉着**，否则下次重构又会悄悄放它过去。
