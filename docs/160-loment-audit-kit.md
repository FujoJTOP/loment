...
```bash
python tools/loment_audit.py --json     # 21/21 通过（本机 C14 是环境红, 见 §2） + loment/build/audit-report.json
...
python tools/loment_audit.py --json # 期望 21/21
```
...
4. **报告**：审计结论请连同 `audit-report.json`（含日期、环境、21 条结果）一起存证；
