...
> **语言线从另一头撞到同一个病灶，而且症状更难看**（2026-09-20 他们的实测，
> 记在 `docs/197` §7.2 与 `tools/nltrans.py` 文件头 —— 这里直接引，不重做）：
> 在**安装版 0.1.4** 的自举驱动上，同一条不是"编不过"，是**编得过、算错**——
>
> ```rust
> fn wr(fd: u64, v: i64) { let x: u64 = (0 - v) as u64; ... }
> fn _start() { wr(1, 0 - 12345); }     // 打出 4294954951（= 2^32 − 12345）
> ```
> 参考实现为同一句发的是 `sub i64 0, 12345`。于是那条规律可以说得更准：
> **无后缀整数字面量在自举驱动里被定型成 u32**，`i64` 上下文里取负就**32 位回绕**；
> 而**运行期的 i64 变量之间没有这个问题**（`let zero: i64 = 0; zero - v` 是对的）——
> 病灶在**字面量的定型**那一步，不在减法本身。他们的规避是"带类型的零"。
>
> 我们两边**独立**撞到同一步，用的是不同的入口（我们这边是"IR 逐字节不一致"，
> 他们那边是"跑出来的数不对"）—— 两条合起来才把病灶钉在"字面量定型"上。
