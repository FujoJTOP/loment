...
| 2 | 聚合返回值不支持 | 明确报 `Unsupported`，不静默错编 |
...
4. **重建不再需要 clang**：本文件的主线用例里 clang 仍出现一次（种子 → stage1）。要连这一步也
   去掉，需要 genesis（可复现、提交进仓库、有哈希的 stage1 二进制，与 Rust 发 stage0 同一做法），
   再把 `bootstrap.sh` 的起点从 clang 换成 genesis。**残留的诚实点**：链条总要有第一个可执行文件，
   genesis 是不可消掉的那个 —— 它可复现、有来源，但它是个二进制，不装作没有。
