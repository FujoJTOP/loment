...
| M82 自举 codegen | `codegen.lomt` 的 `.ll` 与 `lomentc --emit-llvm` 逐字节一致 | ✅ 部分（9 个目标文件；示例级覆盖 12/37，门禁打印缺口分类） |
...
```
python tools/loment_p8_test.py     # 5/5
# 示例覆盖 12/37 字节一致 (9 个 ir_*.lomt 锚点 + toolchain/native_bits/native_mem)
# 缺口分类: 聚合/切片/字符串 18 · 内建 14 · 除法 9 · for 7 · syscall 5 · match/枚举 4 · 能力域 2
```
...
| 真实示例解锁 | `native_bits.lomt`、`native_mem.lomt` 从"有差异"变为**逐字节一致** | ✅ |
...
| M82 | Loment 版 IR 生成 | `.ll` 与 Python 版逐字节一致 | ✅ 部分（标量/控制流/短路/转换/`for`/除法/指针与位域内建；示例级 12/37；见 docs/150） |
