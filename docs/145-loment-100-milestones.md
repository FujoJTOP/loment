...
| M82 自举 codegen | `codegen.lomt` 的 `.ll` 与 `lomentc --emit-llvm` 逐字节一致 | ✅ 部分（9 个目标文件；示例级覆盖 22/37，门禁打印缺口分类） |
...
```
python tools/loment_p8_test.py     # 5/5
# 示例覆盖 22/37 字节一致 (9 个 ir_*.lomt 锚点 + toolchain/native_bits/native_mem/native_str/native/ahci/fuc_node/bytes/allocator/mathutil/user_hello/bootprobe)
# 缺口分类: 聚合 13 · 内建 9 · 除法 6 · for 6 · match/枚举 4 · syscall 3 · 能力域 2
```
...
| M82 | Loment 版 IR 生成 | `.ll` 与 Python 版逐字节一致 | ✅ 部分（标量/控制流/短路/转换/`for`/除法/内建/常量内联；示例级 22/37；见 docs/150） |
