...
```
python tools/loment_p8_test.py     # 10/10
# checker 放行单元: 40/40 无诊断 (已登记缺口 1 个 = 非目标文件)
# 驱动闸门: 负例 6/6 被拒, 正例 1/1 过检
```
...
| `test_m85_driver_checks_before_emitting`：6 个负例全部非零退出 + 带诊断 + **不产出 IR**；正例零退出且产物与参考逐字节一致 | ✅ 闸门打开 |
...
| M85 | 自举编译器跑全部测试 | `lomentc_test` 在自举版上通过 | ✅ 部分（**驱动现在是完整的编译器**：自己装载（`/proc` 取入口 + 递归解析 `use` + 注入预置枚举）、**先 check 再发射**（40/40 语料零诊断、6/6 负例被拒、40/40 目标逐字节一致）、能编译自己并二阶段定点。仍差：① 符号表**全单元扁平**（需给符号加模块作用域）；② `lomentc_test` 那 90 条判据里可映射的部分还没搬到驱动器上跑） |
