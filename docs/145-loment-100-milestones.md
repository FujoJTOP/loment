...
| M85 | 自举编译器跑全部测试 | `lomentc_test` 在自举版上通过 | ✅ 部分（**装载已进驱动**：单二进制按入口路径把整个语料编译一遍、逐个逐字节一致（40/40）；仍差两件事：① 符号表是**全单元扁平**的，兄弟模块同名私有函数会撞名（已记 docs/150，需给符号加模块作用域）；② `checker.lomt` 是 4 条规则的**子集**检查器，会把合法程序判错 —— 得先扩到能放行全部语料才能当闸门） |
