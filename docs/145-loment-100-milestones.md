...
| M82 自举 codegen | `codegen.lomt` 的 `.ll` 与 `lomentc --emit-llvm` 逐字节一致 | ✅ 部分（17 个函数：字面量/参数/二元/比较/一元/调用） |
...
| M82 | Loment 版 IR 生成 | `.ll` 与 Python 版逐字节一致 | ✅ 部分（标量表达式子集：二元/比较/一元/调用；见 docs/150） |
