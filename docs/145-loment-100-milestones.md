...
| M82 自举 codegen | `codegen.lomt` 的 `.ll` 与 `lomentc --emit-llvm` 逐字节一致 | ✅ 部分（5 个目标文件：表达式 + 控制流 + 短路 + 转换 + 实参强制） |
...
| M82 | Loment 版 IR 生成 | `.ll` 与 Python 版逐字节一致 | ✅ 部分（表达式 + 控制流 + 短路 + 转换；见 docs/150） |
