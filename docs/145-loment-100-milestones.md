...
| M100 发布审计 | 审计包就位（一条命令 16/16 + 证据落盘），缺第三方复核 | ⚠️ 0.1.3.4 Alpha |
...
| M100 | 0.1.3.4 Alpha 发布与审计 | 全门禁绿 + 外部审计 | ⚠️ 未达（**门禁侧已齐**: `python tools/loment_audit.py --json` 16/16 条主张通过并落盘证据; 审计包 `docs/160-loment-audit-kit.md` 含主张/不主张/复核步骤/对抗性尝试; **缺第三方复核本身**, 无法由作者自证 —— 这也是本行不能翻 ✅ 的原因） |
