...
| 优先 | 判据 | 形态 | 为什么在它这个位置 |
|---|---|---|---|
| **1** | `loment_eol` | 工具型 | **不新增 Python**；报告文本要逐字复刻（含 10 条截断与三条修法），但它只读 + 打印，**不写盘**（`--fix` 那一路可以先不搬，判据只比无参那一趟） |
| 2 | `loment_editors_test` 第 1 条（vim 语法表 vs TextMate 不漂移） | 检查型 | 纯数据（读两个文本、取词、比集合），但**先要把检查抽成程序** |
| 3 | `loment_src` | 工具型 | 同样要 git，但输出是一个**包**（比 stdout 不够，还要比写出的字节）|
| ✗ | `loment_filetype` | 工具型 | 直接写 **HKCU 注册表** —— Loment 没有那个 syscall。它要么留在 Python，要么改由安装器代劳（这一格**不是**搬，是**换人**）|
