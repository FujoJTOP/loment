...
| clang / LLVM | 发射 IR → 可执行文件 | **地基语言**，本文写作时（0.1.4）明确保留 | ~~不计划去掉~~ **已改为计划去掉**（0.1.4 Alpha2）：参考实现 `tools/lomelf.py` 已能「IR → x86-64 ELF」且与 clang 产物行为逐值一致（`loment_elf_test` 4/4，主张 C19，见 `docs/167`）。**仍待做**：自举侧镜像 `loment/tools/lomelf.lomt`、PE64/去 WSL、构建路径去 Python、genesis（重建不再需要 clang） |
