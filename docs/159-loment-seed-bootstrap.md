...
| clang / LLVM | 发射 IR → 可执行文件 | **地基语言**，本文写作时（0.1.4）明确保留 | ~~不计划去掉~~ **已改为计划去掉**（0.1.4 Alpha2）：参考实现 `tools/lomelf.py` 已能「IR → x86-64 ELF」且与 clang 产物行为逐值一致（`loment_elf_test` 4/4，主张 C19，见 `docs/167`）。**已落地**：自举侧镜像 `loment/tools/lomelf.lomt`（对四个语料与参考逐字节相同，主张 C20）—— 编译用户程序全程无 clang。**已落地（0.1.4 Alpha2）**：自举侧也能重建（`loment_genesis_test`）；链条起点换成**提交进仓库的 genesis** `loment/build/genesis/lomelf-linux-x64.elf` —— `sh loment/bootstrap.sh` 在**没有 clang** 的环境里四条证明全过（主张 C21）。**仍待做**：PE64/去 WSL、构建路径去 Python |
...
| **构建路径** `loment_release.py`（发布清单） | ✅ 已重写 | —（`loment/tools/lomrel.lomt`；glob 展开 + sha256 + 定形 JSON 都与 Python 版**逐字节相同**，判据 `tools/loment_rel_test.py`，主张 **C23**。平台口径：两边都按 **posix 路径的字节序**排 —— 早先镜像的是 Windows 的 `sorted(Path)`（normcase 小写比），于是**同一棵树在两台机器上会排出两份清单**；CI 因此每次都推一笔 `[生成物]`，而那笔机器人推送会把必需检查挂成 `action_required`。2026-09-25 两端一起改成字节序，见 `loment_release.build` 与 `lomrel.cmp_bytes` 的注释） |
