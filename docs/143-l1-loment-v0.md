...
| 模块导入 | **两种写法**。路径形式 `use "a/b.lomt"` 解析相对仓库根或导入文件所在目录；名字形式 `use 名字` 依次在 `loment/lib`、`loment/examples`、`loment/selfhost`、`loment/tools` 下找 `名字.lomt`，**命中必须唯一**（找不到或多处命中都报错，不静默取第一个）。名字形式**只解析 `.lomt`**：L0 布局只能用路径形式，所以不存在「这个名字算 L0 还是 L1」的歧义；同理，**自己目录里的伴生文件要用路径形式**（那几个搜索根之外，名字形式找不到）。同一模块只解析一次；循环导入报错；本模块声明的符号不得与导入符号重名 |
