...
**语义**：它声明**整个程序**的运行模式，而不是某个文件或某个模块的。两种模式的区别是
**能不能用 hosted 层**（libc / 文件系统 / 线程 / 时间 / 网络…）：

| 模式 | 核（freestanding） | hosted 层 |
|---|---|---|
| `std` | ✅ | ✅ |
| `no_std` | ✅ | ⛔ 引用即报错 |

**核**指已有的那些：`alloc` / `free` / `str_*` / syscall 包装 / 平台垫片，以及 `loment/lib/`
里不依赖宿主的那几个。**`no_std` 不是"什么都没有"，是"只有核"** —— Loment 从一开始就是
为写 FujoOS 而立的，freestanding 是它的本相，hosted 是加在上面的那一层（`docs/175` §4）。
...
| **写两次**（任意组合） | 报错 |
| 出现在**非根单元**（被 `use` 进来的库） | 报错 —— **库不许 `choose`** |
| `no_std` 项目里引用 hosted 层的东西 | 报错，**并给出需求来源链** |

**为什么"至多一次"且只在根单元**：它是**整个程序的不变量**，所以编译器能**全局**拦住
"半 hosted、半不是"（那种混合在链接期才炸，最难查）。若放开成按文件/按库声明，就会掉进
Rust `#![no_std]` 那个已知的组合痛点 —— 而 Loment 的单元模型（库 = 目录、依赖 = 源码里的
`use`）会掉进同一个坑。

**库怎么表达自己的需要**：**声明能力需求，而不是模式**。`no_std` 项目里某个库要 hosted 的
文件系统，报错时**报得出是谁拉的**（`docs/168` 的 `lib cap` 已经在算能力需求闭包，带来源链）。
库写"我要什么"，项目定"这个项目给什么"。

**为什么不给默认以外的灵活度（用户 2026-09-17 定）**：省略 = `std`，因为 `std` 是包含核的
超集 —— 忘了写不会编不过，只会拿到多余的宿主依赖；而"要用 freestanding 就必须显式说"
保留了那件事的**可审计性**：Potato 形式对象里把 `mode` 与"是否显式声明"**分开记**，
于是"意图是 freestanding 却没声明"仍然看得见，不必拿强制语法去换。

**与多语法前端的关系**：前端是**模式相关**的。`choose std` 的项目里 Python/Java 语法前端可用
（它们本来就要运行期）；`choose no_std` 的项目里应当**拒绝** Python 前端，或只放行它一个
freestanding 子集 —— 否则会造出一种"看着像 Python、却什么都 import 不了"的东西，
比不支持更糟（`docs/175` §5.4）。

**与冻结面的关系**：这是新顶层形式 = 改冻结面里的"语法与类型规则"，并且模式**必须进 Potato
