...
这一轮移植（`lomc.lomt` 是迄今最大的自举单元：357 个 `let`、108 个 `fn`）逼出了**五个已修**的
问题（自举 checker 类型表没按函数清零、`--debug`+`match` 发非法 IR、`CLAUDE.md` 的假台账原语、
`loment_manual` 从没进过 CI、`loment_audit` 的副本漂移）与**两个仍开放**的自举链缺口（见 §5）。
账在 `docs/150` 的 M81/M82 后置修订，不主张清单见 `docs/160 §2`。
...
1. 自举 checker 在**大单元**里的一条走岔：`lomc.lomt` 里把一个局部命名为 `fn` 会让它报一串
   E002 并退出 1（改名即通过）；**但最小单元同样写法逐字节一致、零诊断** —— 所以**不是**
   "关键字当标识符被误拒"（我最初这么记，已更正，见 `docs/150` M81 后置修订的更正框）。
   形状更像某个**规模相关**的表/游标被这个 token 触发；本次把变量改名绕开，检查器没动。
