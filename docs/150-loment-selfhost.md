...
> 2. **把关键字当变量名 → 检查器假阳性**。`let fn: u64 = ...` 参考实现**接受**（它的词法只有
>    ident/number/string/punct 四类，没有关键字），但自举 checker 是**按 token 字面量**找函数
>    声明的 —— 看到 `let` 后面的 `fn` 就当成函数声明，走岔。这是**假阳性**（拒绝合法程序），
>    比"保守少报"更糟，所以**没有**归进 `docs/158 §4` 那三条"刻意保守偏离"。
>    本次的做法是把变量改名（`fname`），**检查器本身的这条缺口仍开放**：任何用关键字当
>    标识符的程序都会被自举侧误拒。
>
