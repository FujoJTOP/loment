...
### M85 后半 · 批次 2 完成：自举 checker 与参考实现 **60/60 规则等价**（2026-09-12）

上一轮把批次 2 第一段（语句级类型比对）做到 32/60；这一轮把剩下的 28 条补完，实测
**60/60 等价、假阳性 0、码漂移 0**，`loment_rule_parity` 的棘轮预算从 32 调到 60。
补的东西：
