...
### M85 后半 · 为什么 checker 还接不上（两个真缺口）

把 `checker.lomt` 也 `use` 进驱动、让同一个二进制先 `check` 再发射 —— 试过，**现在接不上**，
原因两条都不是"接线"问题：

**① 符号表是**全单元扁平**的，兄弟模块的私有函数会撞名。**
驱动同时装 `codegen.lomt` 与 `checker.lomt` 立刻炸：
...
（一个数形参一个数实参；一个跳过 `<T>` 一个不跳），撞名会静默算错。已把 checker 侧五个都加
`chk_` 前缀绕开并在定义处写明原因，**但这是绕开不是修好**：函数表与字符串全局命名都建立在
"全单元唯一"的假设上，真正的修法是给符号加模块作用域（M11 的"模块/包系统"在**自举版的
名字解析**上还没落地）。这条是自举第一次把"两个模块装进同一个单元"逼出来的。

**② `checker.lomt` 是 M81 的**子集**检查器，会把合法程序判错。**
接上去之后 `pos/ok.lomt` 乃至整个语料都被拒 —— 它只实现了 4 条规则，`use` / 泛型声明 /
`match` / 字符串字面量等构造它不认识，于是报假错误。所以 M85 后半不是"把 checker 接进
驱动"，而是"**把 checker 的覆盖面从 4 条规则扩到能放行全部语料**"（M81 的剩余部分）。

于是：`chk_` 前缀改动保留（无害且澄清了语义差别），驱动的 `check` 闸门**没有**留下 ——
用一个会误报的检查器当闸门只会挡住正确程序。

**假报清单**（把 checker 单独编出来跑 `loment/examples/*.lomt`，13 个文件报错；
这就是"扩覆盖面"的工作清单，按构造归类）：
...
| # | 缺口 | 说明 |
|---|---|---|
| checker 未接进驱动 | 后端已覆盖全语言，但 `checker.lomt` 还没接进同一个可执行文件 —— 所以"自举编译器跑 `lomentc_test`"（诊断判定也一致）还没到 |
