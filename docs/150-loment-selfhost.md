...
### M85 后半 · 缺口①：符号表全单元扁平（**未修**，只绕开）

驱动同时装 `codegen.lomt` 与 `checker.lomt` 时立刻炸：
...
（一个数形参一个数实参；一个跳过 `<T>` 一个不跳），撞名会**静默算错**。已把 checker 侧五个
都加 `chk_` 前缀绕开并在定义处写明原因。

**但这是绕开不是修好**：函数表与字符串全局命名都建立在"全单元唯一"的假设上，真正的修法是
