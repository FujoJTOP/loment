...
**还差什么**：60 条里剩 36 条几乎全落在**表达式类型推断**这一批（批次 2）——
`let`/`return`/赋值/`if`/`while`/`for` 的类型比对、`as` 转换合法性、字段/下标/数组字面量、
`match` 的模式与穷尽性、`?` 误用，以及移动/借用检查（E006/E007/E011/E012）。
这一批需要一个 token 级的类型推断器（参考实现是在 AST 上做 `expr_type`），
是"自举 checker 完全替代参考实现"的最后一段长杆；诚实口径是：
**当前自举 checker 是"声明级规则等价、表达式级规则缺失"的子集**，
`loment_p8_test` 的负例对照里 `unknown_let.lomt` 就是这条边界的实测样本（登记在 `RULE_GAPS`）。
