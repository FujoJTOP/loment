...
` ```rust ` 作围栏。Loment 的原生语法与 Rust **重叠面很大**，关键词/类型/字符串/注释/
函数名这些着色基本一致；只有 `capability` / `guard` / `excluded` / `revocable` 会显示成
普通标识符。

（2026-09-18 起**不再声称"严格子集"** —— 表层语法已不止一种，见 `docs/188`。围栏语言的
理由从来只是**重叠面**上有相同的着色，与"是不是子集"无关，所以这一条照旧成立。）
