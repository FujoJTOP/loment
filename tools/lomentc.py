    ...
    **这条规则的价钱是"整体取消"**：体内**一个**分配出体，整个体这一格就不能归还 ——
    ...
    for k in range(at, end):
        ...
        if word(k, "free"):
            ...
            # 这是"不必取 min"的**证明**：能把前沿往下的只有 `__loment_free`，而它释放的块
            # 要么是体内分配的（那就得在体内写 `free`）、要么是体外分配而在体内释放的
            # （同样得写 `free`）—— 两种都留下 `free` 记号，于是都在这里被挡掉。
            # 而 `min` 需要 `select`，**自举镜像链接器没有 `select`**，包内链接器就是它。
            return False
        if word(k, "alloc"):
            nalloc += 1
    if nalloc == 0:
