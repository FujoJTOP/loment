...
def test_every_tool_is_in_the_release_manifest():
    ...
    方向都会静默出错**（2026-09-22 加这条之前，两边都没有判据管）：
      ...
      * 被跟踪的 `tools/*.py` 不在 `GLOBS` 里 -> 它在仓里、在门禁里**跑**，却**不在
        发布清单上**。这正是 `docs/188` §7.1 加一门表层语法那张表里标着"**没有任何
        判据会抓**"的那一格（`loment-dev-86` 那次是人工看出来的）。
    ...
    **是设计**，不是漏 —— 别照这条判据去"修"它。
    ...
    assert not missing, (
        f"这些被跟踪的 `tools/*.py` 不在 `loment_release.GLOBS` 里 —— 它们随包发不出去，"
        f"而 `--check` 照样绿：{missing}。加进 GLOBS，并**与 `loment/tools/lomrel.lomt` "
        f"的 `globs_text()` 插入同一位置**（两份逐条同序，否则 `loment_rel_test` 红）。")
    assert not dead, (
