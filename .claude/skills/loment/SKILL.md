...
的写法写，**只换拼法、不换语义**。认法有两种：**按后缀**（`.c` / `.cpp` / `.java` / `.cs` /
`.py` / `.go`），或在文件头**显式声明**：

```rust
choose write grammar python     // 必须写在 `module` 之前，一份文件只写一次
```

> ⚠️ **今天两种工具链的接受面不一样，动手前先看这条**：
> **源码仓里的参考编译器**（`tools/lomentc.py`；`loment ir` / `build` 走的也是它）**认**上面
> 两种写法 —— 它在**进程内**把那份源码翻成 Loment 再编，**不调外部编译器**。
> **装好的包（自举工具链）只认 `loment` 拼法**：别的拼法会**明确拒绝**
> （"这份源的 `choose write grammar` 自举侧收不了…"）—— 因为那六个翻译器还没有 Loment
> 孪生（`docs/189` §4.1）。所以"包内写 `.py` 就能跑"这句话**今天不成立**。

别名表是**出厂锁**的（16 条拼法 → 8 个规范名，不可扩展、不可覆盖，`docs/188` §1.1）。
写错是四种报错之一：名字不在表里 / 写在 `module` 之后 / 写两次 / 后面没写名字。

**写这门拼法前先看这几处边界**（实测出来的，能省一轮返工）：

- **数组与指针写不出来**（C 与 Java 都拒："不支持指针/数组声明"；数组字面量撞"不支持裸块"）
  —— 用标量；
- **整数宽度只能用一个**：混用会在检查那一步**硬失败**（`docs/188` §7.1.1）；
- **没有按位取反 `~`**（Loment 的一元运算符只有 `-` 与 `!`）：要它得**按宽度**写
  `v ^ -1`（i32）或 `v ^ 255`（u8）—— 前端会点名拒，并把这两条出路写在话里；
- **Python 拼法里 `int` 就是 `i64`**（有界、溢出**回绕**），不是 CPython 的任意精度 ——
  这件事在源码层**看不见**，别指望它报错。
...
| 函数 | `fn f(a: u32, b: str) -> u32 { ... }`（无返回写 `fn f()`）；**形参最多 10 个**（自举侧**布局**的硬约束: 一张形参表只有 10 槽。超限的失败形态有两条: 形参与实参**数目对不上**是正常的 E3；**实参多于 10 个的调用点**走 `panic` —— 退出码 132、**不打诊断**。撞上"没报错但退出码 132"时先数实参） |
...
> **"把开关装进本机"那套（`lock` / `chooseunlock.lomlock`）还没有**（`docs/182` §3 的计划）。
> 它挡在一条尚未定的规矩上：锁是**本机**的，而本仓的身份是"同一份源 → 同一串字节"——
> 锁上之后，同一份源在两台机器上会编出不同的东西。今天要让某个开关成为默认，
> **只能写在源码里**。（包管理器的锁文件 `lompi.lock` 是**另一回事**，见 lompi 的指南。）
